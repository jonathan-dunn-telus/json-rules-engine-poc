...
async function start () {
  ...
  const cesProductGroupRule = {
    ...
    event: { type: 'ces-product-group' },
    priority: 10, // IMPORTANT!  Set a higher priority for the drinkRule, so it runs first
    // onSuccess: async function (event, almanac) {
    //  almanac.addRuntimeFact('ces-product-group', true)
    // },
    //onFailure: function (event, almanac) {
    //  almanac.addRuntimeFact('ces-product-group', false)
    //}
  }
  engine.addRule(cesProductGroupRule)
